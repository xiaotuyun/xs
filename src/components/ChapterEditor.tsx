...
export const ChapterEditor: React.FC<ChapterEditorProps> = ({
  ...
  const handleAiPolish = async () => {
    ...
    const cleanedEditorText = cleanPreambleText(content);
    const instructionText = polishInstruction.trim();

    if (!cleanedEditorText && !summary.trim() && !instructionText) {
      setError('请输入或在上方填写本章剧情大纲/润色要求，AI 才能为您进行创作精修！');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="bg-white p-3 sm:p-4 rounded-2xl border border-stone-200 shadow-xs flex flex-wrap items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center space-x-2 sm:space-x-3 w-full sm:w-auto">
          <select
            aria-label="选择章节"
            value={currentChap.id}
            onChange={(e) => onSelectChapter(e.target.value)}
            className="flex-1 sm:flex-none bg-stone-50 border border-stone-300 text-stone-800 text-[13px] sm:text-sm font-medium rounded-xl px-2.5 sm:px-3.5 py-1.5 sm:py-2 focus:ring-2 focus:ring-amber-500 outline-none max-w-[200px] sm:max-w-xs truncate"
          >
          ...
          <span className="text-[11px] sm:text-xs text-stone-500 whitespace-nowrap">
            <span className="hidden min-[400px]:inline">字数: </span><strong className="text-stone-900">{getPureWordCount(content).toLocaleString()}</strong>
          </span>
        </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 w-full sm:w-auto justify-end sm:justify-start">
          <div className="flex items-center space-x-1 sm:space-x-2">
            {prevChap && (
              <button
                ...
                className="p-2 sm:px-3 sm:py-2 border border-stone-300 text-xs font-medium rounded-xl text-stone-700 bg-white hover:bg-stone-50"
                title="上一章"
              >
          ...
          <div className="flex items-center space-x-2 flex-1 justify-end">
            {syncToDiskStatus === 'syncing' && (
              <span className="text-[10px] sm:text-xs text-amber-600 flex items-center space-x-1 font-black bg-amber-50 px-1.5 sm:px-2.5 py-1 sm:py-1.5 rounded-lg border border-amber-200/55 animate-pulse">
                <Loader2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 animate-spin" />
                <span className="hidden min-[400px]:inline">同步中</span>
              </span>
            )}
            {syncToDiskStatus === 'success' && (
              <span className="text-[10px] sm:text-xs text-emerald-700 flex items-center space-x-1 font-black bg-emerald-50 px-1.5 sm:px-2.5 py-1 sm:py-1.5 rounded-lg border border-emerald-200/55">
                <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                <span className="hidden min-[400px]:inline">同步(TXT)</span>
              </span>
            )}
            ...
            <button
              ...
              onClick={handleSave}
              disabled={isSaving}
              className="inline-flex items-center px-3 sm:px-4 py-1.5 sm:py-2 bg-amber-600 hover:bg-amber-700 text-white text-[13px] sm:text-sm font-medium rounded-xl shadow-sm transition-colors space-x-1 sm:space-x-2 cursor-pointer shrink-0"
            >
              {savedStatus ? (
                <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 bg-white rounded-2xl border border-stone-200 p-6 sm:p-8 shadow-xs space-y-6">
          ...
          <div className="space-y-2">
            ...
            {/* Quick AI Action Bar for Mobile/Tablet */}
            <div className="lg:hidden flex flex-wrap sm:flex-nowrap gap-2 pb-2">
              <button
                type="button"
                onClick={handleAiGenerate}
                disabled={isGenerating}
                className="flex-1 min-w-[120px] py-2.5 px-3.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold rounded-xl shadow-sm transition-all flex items-center justify-center space-x-1.5 disabled:opacity-50 active:scale-95 shrink-0 cursor-pointer"
              >
                {isGenerating ? (
                  <>
              ...
              <button
                ...
                className="flex-1 min-w-[120px] py-2.5 px-3.5 bg-stone-900 hover:bg-stone-800 text-white text-xs font-semibold rounded-xl shadow-sm transition-all flex items-center justify-center space-x-1.5 disabled:opacity-50 active:scale-95 shrink-0 cursor-pointer"
              >
                {isContinuing ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
              ...
              <button
                ...
                className="flex-1 min-w-[120px] py-2.5 px-3.5 bg-stone-100 hover:bg-stone-200 text-stone-800 text-xs font-semibold rounded-xl transition-all flex items-center justify-center space-x-1.5 active:scale-95 shrink-0 cursor-pointer"
              >
      ...
      {showPolishModal && (
        <div className="fixed inset-0 bg-stone-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-stone-500 leading-relaxed">
              可在此输入<strong>修改要求/避坑指令</strong>，或直接<strong>粘贴草稿片段并在末尾加上修改要求</strong>，AI 将智能解析并全量润色扩写：
            </p>
            ...
            <textarea
              ...
              placeholder={`支持直接粘贴草稿并加上要求，例如：\n徐微坐在蒲团上，周身灵气汇聚……（在此粘贴草稿段落）\n\n修改要求：不要出现混沌灵光等，灵光就是灵光，主角依然是后天跟脚`}
