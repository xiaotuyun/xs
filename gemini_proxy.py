...
def list_models():
    try:
        ...
        clean_msg = err_str
        is_permission_denied = "permission" in err_str.lower() or "403" in err_str or "PERMISSION_DENIED" in err_str
        if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
            clean_msg = "请求过于频繁或 API 配额已耗尽 (429 RESOURCE_EXHAUSTED)。"
        elif is_permission_denied:
            clean_msg = "API Key 或代理接口无模型列表查询权限 (Permission Denied)。"
        elif "RemoteProtocolError" in err_str or "Server disconnected" in err_str:
            clean_msg = "网络代理连接中断，请检查本地代理工具（如 SOCKS5/Clash）是否稳定。"
        return jsonify({"success": False, "error": clean_msg, "permission_denied": is_permission_denied}), 200
